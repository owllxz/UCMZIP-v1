...
lista *recorrer_lista(lista *list){
	while(list != NULL){
		...
		else if(list -> caracter == '\0'){
			printf("%d Nulo\n", list -> value);	
		}
		else if(list -> caracter == ' ')
			printf("%d Espacio\n", list -> value);
		else{
...
int buscar(lista *list, char caracter){
 	lista *nodo_a;
 	nodo_a = list;
 	int encontrado = 0;
 	while(nodo_a != NULL){
...
lista *agregar_inicio(lista *list, int value, char caracter){
	lista *nodo_a;
	nodo_a = crear_nodo();
	nodo_a -> value = value;
...
char caracter(lista *list,int pos){
	int i = 0;
	if(list == NULL){
